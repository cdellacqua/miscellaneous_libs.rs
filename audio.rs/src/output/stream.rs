...
pub trait StreamProducer<const SAMPLE_RATE: usize, const N_CH: usize> {
	fn data_producer(&mut self, chunk: InterleavedAudioBuffer<SAMPLE_RATE, N_CH, &mut [f32]>);
	fn on_error(&mut self, reason: &str);
}

pub struct OutputStreamBuilder<const SAMPLE_RATE: usize, const N_CH: usize> {
	device_name: Option<String>,
	producer: Box<dyn StreamProducer<SAMPLE_RATE, N_CH> + Send + 'static>,
}
...
impl<const SAMPLE_RATE: usize, const N_CH: usize> OutputStreamBuilder<SAMPLE_RATE, N_CH> {
	...
	pub fn build(self) -> Result<OutputStream<SAMPLE_RATE, N_CH>, AudioStreamBuilderError> {
		...
		Ok(OutputStream::new(device, config, self.producer))
	}
}
				...
				device
					.build_output_stream(
							...
							move |output: &mut [f32], info| {
								...
								shared.with_lock_mut(
									|StreamState {
										...
										producer.data_producer(InterleavedAudioBuffer::new(output));
						...
						move |err| {
							...
							shared.with_lock_mut(|StreamState { producer, .. }| {
								producer.on_error(&err.to_string());
							});
						},
						None,
					)
